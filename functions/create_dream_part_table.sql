...
DROP FUNCTION IF EXISTS dream.create_dream_part_table(col_param text, tablename_param text, indexname_param text) ;
CREATE OR REPLACE FUNCTION dream.create_dream_part_table(col_param text, tablename_param text, indexname_param text) 
...
BEGIN
	...
	WHERE	col like col_param) --Kun kolonnenavne med ydelse starter med "y" og har i alt præcis karakterer, fx y_201344
		, ',');
...
EXECUTE	$code$
	...
	drop table if exists $code$ || tablename_param || $code$;
	create unlogged table $code$ || tablename_param || $code$ (
			lobenr text,
			$code$
			|| col_list_all_text ||
			$code$
...
EXECUTE	$code$
	copy $code$ || tablename_param || $code$ from '/Users/lokesonne/Documents/data/dream_out.csv'	
	with 
	delimiter ','
	encoding 'utf8'
	csv;

CREATE INDEX $code$ || indexname_param || $code$ on $code$ || tablename_param || $code$ (lobenr);
